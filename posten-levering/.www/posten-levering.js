class PostenLevering extends HTMLElement {
  set hass(hass) {
    ...
    try { 
      const dates = Array.isArray(state.state) ? state.state : JSON.parse(state.state);
      ...
      let dateList = '<ul class="posten-list">';
      dateList += `<li class="posten-entry"><small class="posten-text">Ingen datoer</small> <small>${state.state}</small></li>`;
      dateList += `<li class="posten-entry"><small class="posten-text">Prøv å restart add-on eller sjekk logger</small> <small>${state.state}</small></li>`;
      dateList += '</ul>';
